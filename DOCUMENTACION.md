...
- **Secuencia en examen normal:** tras `agudeza_alcanzada` de L, el test `binocular` (`testActual.ojo: 'B'`) aplica ajuste final con **ambos ojos abiertos**. Línea base: esférico fino por ojo; cilindro y ángulo **por campo** vía `resolverCilindroYAnguloOjo` (cilindro del test monocular si existe; ángulo del test de ángulo si existe, si no recalc) — ver `DEFINICIONES_EXAMEN_BINOCULAR.md` §2.1.
...
- **Comparaciones (esférica → cilíndrica opcional):** en cada ronda el backend aplica la **variante** (0,50 D hacia el cero) y emite un **único** `hablar` que concatena el aviso de *otro par* y la pregunta *anterior / actual*; en ese turno el paciente ya viste con la variante puesta, alineado con el protocolo. Las respuestas se envían con `interpretacionComparacion`. **Pasos omitidos** si la variante no mueve ningún ojo (esfera 0/0 o cilindro 0/0 en ambos ojos) — ver `DEFINICIONES_EXAMEN_BINOCULAR.md` §7.
...
- **Resultado:** `resultados.R.binocular` y `resultados.L.binocular` como `{ esfera, cilindro, angulo }` por ojo. Al confirmar (`confirmarResultadoBinocular`), se envía **siempre** al foróptero `foropteroDesdeRx(rxFinal)` (ambos ojos `open`), sin ramificar por preferencia `anterior` / `actual` / `igual` — el hardware puede no-op si ya está en esa Rx; el mensaje de cierre puede ir en paralelo (fire-and-forget). Plan/QA: `PLAN_FEEDBACK_CLIENTE_BINOCULAR_FIN.md`, commit `0163239`, `registros-examen/examen-registro-22.csv`. Modo prueba: `testbin` usa `valoresRecalculados` completo; ver `examenprueba.md`.
